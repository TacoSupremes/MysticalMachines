...
import com.tacosupremes.mysticalmachines.common.block.tile.TileMarker;
		...
		if(heldItem.getItem() == ModItems.builderScroll && state.getValue(active)){
			...
			while(y_ != yO){
				while(x_ != xO){
					...
					b.setInteger("BLOCK"+index, Block.REGISTRY.getIDForObject(w.getBlockState(pos.add(x_, y_, z_)).getBlock()));
